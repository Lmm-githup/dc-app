 ...
 <div style="height:100%">
    ...
    <div class="list" v-if="list.length > 0">
      <van-cell is-link class="list-item" @click="goRoute(item)" v-for="(item, index) in list" :key="index">
        <template slot="title">
          <h3 class="title"><img src="/src/assets/images/me/u13128.png"/>订单编号：{{item.orderNo}}</h3>
        </template>
        <template slot="label">
...
.list-item {
  ...
  .title{
    font-size: 20px;
    font-weight: 700;
    img{width:.8rem;vertical-align: -0.2rem;margin-right:.1rem;}
  }
  .price {
    font-size: 14px;
    margin-top: 20px;
...
.van-cell{width:95% !important;margin:10px auto;border-radius:10px;}
</style>
