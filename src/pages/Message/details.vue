    ...
    <div class="container">
        ...
        <div v-for="(item, index) in list" :key="index">
          <div class="coup-content">
            <div class="coup-header">
              <span style="font-size:16px;font-weight:700;color:#312640">{{item.voucherName}}</span>
              <span style="color:#FFA41A">未使用</span>
            </div>
          ...
          <div style="color:#5c5c5c">劵有效期：{{item.beginDate | dateFormat('YYYY/MM/DD')}} - {{item.endTime | dateFormat('YYYY/MM/DD')}}</div>
            ...
            <div class="item-footer" v-if="remindStatus == '未处理'">
              <a class="handle-btn phone-btn" :href="'tel:' + (/^1[3456789]\d{9}$/.test(item.memberPhone) ? item.memberPhone : item.homeMobile)">
                <i class="fa fa-mobile" aria-hidden="true"></i>
                打电话
              </a>
              <a class="handle-btn phone-stop" @click="openDia(item)">
                <i class="fa fa-bell" aria-hidden="true"></i>
                打电话提醒结果
              </a>
            </div>
      ...
      <div v-if="remindTypeCode == 2 || remindTypeCode == 3">
        <div v-for="(item, index) in list" :key="index">
          ...
          <div class="item-footer">
            <a class="handle-btn" :href="'tel:' + (/^1[3456789]\d{9}$/.test(item.memberPhone) ? item.memberPhone : item.homeMobile)">
              <i class="fa fa-mobile" aria-hidden="true"></i>
              打电话
            </a>
...
/deep/ .vux-header .vux-header-title{color:#fff !important}
.footer {
  position: fixed;
  bottom: 0;
  left: 0;
...
.item-footer {
  ...
  .phone-btn{border:1px solid #037de0;width:2.3rem;line-height:.77rem;border-radius:5px;color:#037de0 !important;margin-left:29%}
  .phone-stop{border:1px solid #ccc;width:3.6rem;line-height:.77rem;border-radius:5px;margin-left:6%}
